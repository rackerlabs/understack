...
[Argo Events][argo-events] operates within a dedicated namespace (argo-events),
while the actual sensors and operations run in another dedicated
namespace (argo-events), WAIT WHAT THAT'S THE SAME,
to ensure proper security isolation and resource control. This separation
is provided by [Argo Events][argo-events] but poorly documented upstream which
they call [Managed Namespace][argo-events-managed-ns].
...
We currently have the [Sensor][argo-events-sensor]s living in the (openstack)
namespace because they need to be co-located with the [EventSource][argo-events-eventsource]
they are using.
...
[argo-events-managed-ns]: <https://argoproj.github.io/argo-events/managed-namespace/>
