...
from understack_workflows.main.sync_server import get_args
from understack_workflows.main.sync_server import get_ironic_node
from understack_workflows.main.sync_server import update_ironic_node
...
def mock_args(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["pytest", read_json_samples("json_samples/event-interface-update.json")],
    )
...
def test_update_ironic_node(fake_client):
    ...
    drac_ip = json.loads(read_json_samples("json_samples/event-interface-update.json"))[
        "data"
    ]["ip_addresses"][0]["host"]

    patches = [
        {"op": "add", "path": "/name", "value": "1327198-GP2S.3.understack.iad3"},
