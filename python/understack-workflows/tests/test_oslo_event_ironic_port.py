...
class TestCableManagement:
    ...
    def mock_nautobot(self):
        ...
        # Mock switch device
        switch_device = Mock()
        switch_device.id = "switch-123"
        nautobot.dcim.devices.get.return_value = switch_device

        # Mock switch interface
        switch_interface = Mock()
        switch_interface.id = "switch-interface-456"
        nautobot.dcim.interfaces.get.return_value = switch_interface
    ...
    def test_cable_management_create_new_cable(
        ...
        # No existing cable
        mock_nautobot.dcim.cables.get.return_value = None
        ...
        # Verify calls
        mock_nautobot.dcim.devices.get.assert_called_with(name="switch1.example.com")
        mock_nautobot.dcim.interfaces.get.assert_called_with(
            device="switch-123", name="Ethernet1/1"
        )
        mock_nautobot.dcim.cables.get.assert_called_with(
            termination_a_type="dcim.interface",
            termination_a_id="server-interface-789",
        )
    ...
    def test_cable_management_update_existing_cable(
        ...
        existing_cable.id = "cable-123"
        existing_cable.termination_b_type = "dcim.interface"
        existing_cable.termination_b_id = "wrong-interface-123"
        mock_nautobot.dcim.cables.get.return_value = existing_cable
    ...
    def test_cable_management_switch_not_found(
        ...
        """Test when switch device is not found."""
        # Mock switch device not found
    ...
    def test_cable_management_switch_interface_not_found(
        self, mock_nautobot, mock_server_interface, test_event
    ):
...
class TestHandlePortCreateUpdate:
    ...
    def test_handle_port_create_update_with_remote_info(
        ...
        # Mock switch and cable components for cable management
        switch_device = Mock()
        switch_device.id = "switch-123"
        mock_nautobot.dcim.devices.get.return_value = switch_device

        switch_interface = Mock()
        switch_interface.id = "switch-interface-456"

        # Mock the interface get calls - first two for server
        # interface lookup, then one for switch interface
        def mock_interface_get(*args, **kwargs):
            if "id" in kwargs:
                # Server interface lookup by ID
                return None
            elif "device" in kwargs and "name" in kwargs:
                # Could be server interface lookup by device+name
                # OR switch interface lookup
                if kwargs["device"] == "switch-123":
                    # Switch interface lookup
                    return switch_interface
                else:
                    # Server interface lookup
                    return None
            return None

        mock_nautobot.dcim.interfaces.get.side_effect = mock_interface_get

        mock_nautobot.dcim.cables.get.return_value = None  # No existing cable
