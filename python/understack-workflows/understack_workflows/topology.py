def pxe_interface_name(nautobot_data: dict) -> str:
    ...
    """
    switches = switch_connections(nautobot_data)
...
def switch_connections(nautobot_data: dict) -> dict:
    return {
        i["name"]: i["connected_interface"].get("device", {}).get("name")
        for i in nautobot_data["interfaces"]
        if i.get("connected_interface")
    }
