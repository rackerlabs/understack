...
class Nautobot:
    ...
    def add_port_vlan_associations(
        ...
        current_tagged_vlans = {
            tagged_vlan["vid"]
            for tagged_vlan in current_state.get("tagged_vlans", [])
        }
        ...
        payload: dict = {
            "tagged_vlans": [
                _vlan_payload(vlan_group_name, vlan_id)
                for vlan_id in tagged_vlans
            ],
        }

        if native_vlan_id is not None:
            payload["untagged_vlan"] = _vlan_payload(
                vlan_group_name, native_vlan_id
            )
