...
class CiscoAsa(base.L3ServiceProvider):
    ...
    def _process_floatingip_update(self, resource, event, trigger, payload):
        ...
        asa_inside_inf = None
        asa_outside_inf = "OUTSIDE"
        if inside_port_info:
            inside_net_id = inside_port_info["network_id"]
            LOG.debug("floatingip_update inside_port_info %s", inside_net_id)
            if inside_net_id:
                LOG.debug("floatingip_update before get_network")
                try:
                    inside_net = self.core_plugin.get_network(
                        context, inside_net_id, fields=["description"]
                    )
                except Exception as e:
                    LOG.debug("floatingip_update get_network failed: %s", str(e))
                    raise
                LOG.debug("floatingip_update after get_network %s", inside_net)
                asa_inside_inf = inside_net["description"].split("=")[-1]
        ...
        LOG.debug(
            "Request to %s floating IP %s via router %s/%s to %s on %s",
            action_msg,
            float_ip_addr,
            router_id,
            asa_mgmt,
