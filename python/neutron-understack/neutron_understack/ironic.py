...
class IronicClient:
    ...
    def available_node_for_resource_class(
        ...
        Ironic filters server-side by ``driver=netdev``, ``resource_class`` and
        ``provision_state=available``, so any returned node is a netdev appliance
        in the interchangeable pool for this flavor. Selection is first-match;
        there is no scheduling/ranking.
    ...
    def _return_node_to_available(self, node: str | BaremetalNode) -> None:
        ...
        * ``active``     -> ``undeploy`` (triggers cleaning), which also tears
          down the instance association;
        * anything else (e.g. ``adopt failed``, ``adopting``) -> leave for
        ...
        if state == _STATE_AVAILABLE:
            LOG.info("Node %s already available; nothing to return", node_id)
            return
