...
from neutron.objects.ports import Port, PortBindingLevel
...
class UnderStackTrunkDriver(trunk_base.DriverBase):
    ...
    def _add_subports_networks_to_parent_port_switchport(
        ...
        allowed_vlan_ids = self._collect_vlan_ids_to_add(
    ...
    def _delete_network_segment_if_not_used(self, segment_id: str) -> NetworkSegment:
    ...
    def _collect_vlan_ids_to_remove(self, subports: list[SubPort], host: str) -> set:
        vlan_ids_to_remove = set()
        for subport in subports:
            subport_binding_level = self._delete_binding_level(subport["port_id"], host)
            network_segment = self._delete_network_segment_if_not_used(
    ...
    def _handle_subports_removal(
        ...
        vlan_ids_to_remove = self._collect_vlan_ids_to_remove(subports, binding_host)
        self.nb.remove_port_network_associations(
            connected_interface_id, vlan_ids_to_remove
        )
