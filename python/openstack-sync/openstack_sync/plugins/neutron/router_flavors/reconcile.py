...
def find_matching_profile(profiles: list[Any], meta_info: Any) -> Any | None:
    """Return the operator-owned profile matching *meta_info*, if any.
    ...
    ``get_flavor_next_provider`` picks an arbitrary binding (``objs[0]``), so a
    stale one can end up serving routers with the wrong ``meta_info``.

    An unowned profile that happens to match is left completely alone -- not
    adopted by stamping the marker onto it, which would enrol somebody else's
    profile into ``prune`` for eventual deletion -- and ``ensure_profile``
    creates a dedicated managed profile alongside it.
    """
    unowned: list[str] = []
    for profile in profiles:
        ...
        if is_managed_service_profile(profile):
            return profile
        unowned.append(str(get_value(profile, "id", default="<unknown>")))
