...
CRD_KIND = os.environ.get("NEUTRON_ROUTER_FLAVOR_CRD_KIND", "NeutronRouterFlavor")
CRD_BINDING_NAME = os.environ.get(
    "NEUTRON_ROUTER_FLAVOR_CRD_BINDING_NAME",
    "neutron-router-flavors",
)
CRD_RESOURCE = os.environ.get(
    "NEUTRON_ROUTER_FLAVOR_CRD_RESOURCE",
    "neutronrouterflavors.neutron.understack.rackspace.net",
)
CRD_NAMESPACE = os.environ.get("NEUTRON_ROUTER_FLAVOR_NAMESPACE") or os.environ.get(
    "POD_NAMESPACE"
)
STATUS_ENABLED = env_bool("NEUTRON_ROUTER_FLAVOR_STATUS_ENABLED", True)
...
# - service_profile.meta_info["_understack_router_flavor_source"]=<source>:
...
def validate_config(flavors: Any, source: str) -> list[dict[str, Any]]:
