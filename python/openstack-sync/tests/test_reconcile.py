...
Covers the profile cache, create-or-reuse by ``(driver, meta_info)``, the
owned-only reuse rule, drift reporting on reused profiles, the flavor
...
# ---------------------------------------------------------------------------
# Only operator-owned profiles are reused
# ---------------------------------------------------------------------------
...
def test_find_matching_profile_ignores_unowned_match():
    ...
    assert reconcile.find_matching_profile([unowned], meta_info) is None
...
def test_ensure_profile_creates_owned_profile_instead_of_reusing_unowned():
    ...
    unowned = _make_profile("adhoc-profile", meta_info=meta_info, managed=False)
    created = _make_profile("new-profile", meta_info=meta_info)
    conn = _create_conn(created, existing=[unowned])
    ...
    new_meta = plugin_common.normalize_meta_info(
        conn.network.create_service_profile.call_args.kwargs["meta_info"]
    )
    for key, value in markers.OPERATOR_META_INFO_MARKERS.items():
...
def test_ensure_profile_never_adopts_an_unowned_profile():
    """The unowned profile is left alone, not stamped with the marker.

    Adopting it would enrol somebody else's profile into the prune sweep, which
    deletes owned unattached profiles -- an irreversible side effect on a
    resource the operator did not create.
    """
    meta_info = {"vni_alloc": "auto"}
    unowned = _make_profile("adhoc-profile", meta_info=meta_info, managed=False)
    conn = _create_conn(_make_profile("new-profile"), existing=[unowned])

    reconcile.ensure_profile(conn, _NAME, _profile_spec(meta_info=meta_info), {}, [])
...
def test_profile_created_beside_unowned_match_can_later_be_unbound():
    ...
    Resolve a profile while an unowned match exists, then reconcile the flavor
    against a different spec. The profile bound earlier must be unbindable,
    which holds only because the operator created and owns it.
    """
    meta_info = {"vni_alloc": "auto"}
    unowned = _make_profile("adhoc-profile", meta_info=meta_info, managed=False)
    created = _make_profile("prof-a", meta_info=meta_info)
    conn = _create_conn(created, existing=[unowned])
    ...
    bindings = _bindings_conn(flavor, {"prof-a": bound})
