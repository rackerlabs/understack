...
[https://github.com/rackerlabs/cinder](https://github.com/rackerlabs/cinder)
...
git remote add rackerlabs https://github.com/rackerlabs/cinder
...
(pointing at `openstack/cinder`, not `origin`) and a remote named `rackerlabs`.

```bash
scripts/git-understack-rebase ~/work/cinder 2026.1
```
...
- **MODIFIED** — same `Change-Id` but the content differs. This can happen when
  a patch was cherry-picked, then tweaked before it merged upstream, so git could
  not drop it by patch-id and it conflicted.
...
```bash
# Fetch all branches from upstream (openstack/cinder)
...
Following the backporting workflow above results in a PR like https://github.com/rackerlabs/cinder/pull/XXX:
...
- OpenStack upstream master branch (`openstack/cinder` - origin/master)
