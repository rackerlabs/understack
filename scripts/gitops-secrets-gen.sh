...
function usage() {
    ...
    echo "" >&2
    echo "OpenStack Service Account Options:" >&2
    echo "  - Traditional: Uses SealedSecrets generated by this script" >&2
    echo "  - ESO: Uses External Secrets Operator (see docs/deploy-guide/secrets-eso-setup.md)" >&2
    echo "" >&2
    echo "This script generates database/RabbitMQ secrets regardless of the approach." >&2
    echo "For ESO, OpenStack service account passwords are managed externally." >&2
    exit 1
}
...
##
## NOTE: If using External Secrets Operator (ESO) for OpenStack service accounts,
## the keystone service account passwords will be managed by ESO instead of
## generated here. See docs/deploy-guide/secrets-eso-setup.md for ESO configuration.
## This script still generates the required database and RabbitMQ credentials.
##
