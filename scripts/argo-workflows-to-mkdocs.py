...
def main():
    ...
    for workflow in subfolders:
        ...
        try:
            workflow_files = [f"{templates_path}/{fn}" for fn in os.listdir(templates_path)
                            if any(fn.endswith(ext) for ext in included_extensions)]
            log.debug(f"found workflow files: {workflow_files}")
        except FileNotFoundError:
            log.warning(f"Directory: {templates_path} unable to find workflow templates")
            continue
        ...
        generate_mermaid(workflows.values(), nodes, workflow.name, output_file, workflow_readme)
...
def make_workflow(w_yaml):
    ...
    for n_yaml in w_yaml['spec']['templates']:
        ...
        for t_yaml in steps_yaml or []:
            ...
            if type(t_yaml) == list:
                t_yaml = t_yaml[0]
                log.debug(f"t_yaml is a list? converted it.")
