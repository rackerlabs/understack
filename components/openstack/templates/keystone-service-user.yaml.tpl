...
{{- $domain_name := eq $user.usage "admin" | ternary "default" "service" }}
{{- $project_name := eq $user.usage "admin" | ternary "admin" "service" }}
---
  ...
  target:
    ...
    template:
      ...
      data:
        ...
        OS_PROJECT_DOMAIN_NAME: {{ $domain_name | quote }}
        OS_PROJECT_NAME: {{ $project_name | quote }}
        OS_USER_DOMAIN_NAME: {{ $domain_name | quote }}
