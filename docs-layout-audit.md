...
Does the current documentation layout serve the three audiences the project has?

- **operators** deploying and running UnderStack,
...
five thin `component-*.md` blurbs. An evaluator reading it end-to-end learns
...
Six tabs, one obvious front door per audience. `Home` is unavoidably a tab, so
the real budget is five.

| Tab | Audience | Why |
| --- | --- | --- |
| **Home** | router | Already has `hide: [navigation, toc]` and a card grid. Becomes an explicit three-way audience router. |
| **Introduction** | evaluators | What this is, how it is shaped, whether it fits. Read end-to-end in fifteen minutes, then left. |
| **Deploy** | operators, day 0 | A linear install narrative with a beginning and an end. |
| **Operations** | operators, day 2 | The runbook library you land in from a pager, plus the small "using the cloud" set. |
| **Contributing** | developers | Does not exist today. Front door for the 58 external files. |
...
`User Guide` is absorbed into Operations as a "Using the Cloud" section rather
than keeping its own tab: it is five pages averaging ~110 lines,
`operator-guide/index.md` already treats `openstack-cli.md` as its prerequisite,
and one of its five pages is a duplicate. That buys back the tab Contributing
needs. If six still feels like too many, the fallback is merging Deploy into
Operations — **not** merging Introduction into Home, since that would cost an
audience its front door, which is the whole point.
...
- `user-guide/` also keeps its path, nav-nested under Operations.
...
- **Phase 1 — nav rewrite plus the three front doors.** This is the phase that
  ...
  on-disk paths only** — no file moves. Add `docs/introduction/index.md`,
  `docs/contributing/index.md`, `docs/reference/index.md` and a
  `docs/operator-guide/troubleshooting.md` hub. Rewrite the "Getting Started"
  card on `docs/index.md` into three audience cards (cheapest high-value edit in
  the whole plan). Delete `design-guide/intro.md` and `user-guide/index.md`. Add
  `mkdocs-redirects`. Revertable by reverting one file, with zero external
  breakage.
- **Phase 2 — content merges.** Fold the five `component-*.md` into one
  `introduction/components.md`; `secrets.md` into `secrets-eso-setup.md`; the two
  `server-firmware-update.md` into one. Move `networking.md` to
  `deploy-guide/provisioning-network.md` and `kubernetes.md` to
  `deploy-guide/tools.md`, which dissolves the naming collision. Retitle the
  colliding H1s so the three OpenStack Helm pages become "Why We Diverge from
  OpenStack Helm", "openstack-helm (component)" and "Troubleshooting OpenStack
  Helm".
- **Phase 3 — dissolve `design-guide/`.** Hardware schemas to `reference/`, the
  four design deep-dives to `contributing/design/`, architecture to
  `introduction/`. Needs the `Makefile` edit for the generated sample config path
  and the two `ansible/roles/nova_flavors/` link fixes. Add the missing
  reciprocal links between the reference and how-to hardware pages.
